...
##########################
# TO DO IN PREPROCESSING SCRIPT
# downsample
# add baseline correction (different for time-frequency and ERP analysis)
##########################
...
grandaverage_TFCE = np.mean([epochs_equal_manmade.get_data().transpose(0, 2, 1), # transpose because the cluster test requires channels to be last
     epochs_equal_natural.get_data().transpose(0, 2, 1)], axis = 0, keepdims = True)
...
all_epochs_perm = np.mean(all_epochs, axis = 0)[:, :, temporal_mask]
