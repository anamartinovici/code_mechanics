...
# pattern_AutoReject = '_AutoReject_epo.fif' # include only epochs with all conditions
# filenames_AutoReject = [item for item in filenames if pattern_AutoReject in item] # list of all '_AutoReject_epo.fif' files
...
for path in filenames: # loop through filenames
    ...
    manmade = mne.read_epochs(path, preload = True)
    df = manmade.to_data_frame()
    
    df.to_csv(
        path_or_buf = '/home/aschetti/Documents/Projects/code_mechanics/data/processed_data/ERP/data_frames/sub-003.csv',
        sep = ',',
        na_rep = '', 
        float_format=None, 
        columns=None,
