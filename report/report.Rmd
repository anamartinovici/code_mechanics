...
-   **reproducibility**: `reticulate` *v*1.24 [@reticulate]

# ANA COULD YOU PLEASE ADD HERE INFORMATION ABOUT MAKE AND THE R PACKAGES YOU USED FOR REPRODUCIBILITY (SEE \`RETICULATE\` EXAMPLE)? NO NEED TO ADD REFERENCES VIA RSTUDIO, BUT IF YOU WRITE DOWN DOIs YOU'LL MAKE MY LIFE EASIER :)

# Software - Ana

EEG preprocessing was carried out using `MNE-Python` *v*1.0.2 [@gramfort2013] in *Python* *v*3.8.13 [@vanrossum-2009]. Analysis, visualization, and report generation were carried out in *R* *v*4.2.0 [@R-base] and *RStudio IDE* *v*2022.02.2+485 [@R-studio]. We used the following *R* packages (and their dependencies):
...
`remotes` *v*2.4.2 [@remotes]

`GNU Make` *v*4.3

`git` *v*2.31.1.windows.1
